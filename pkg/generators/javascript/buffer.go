...
// BufferBuilder is used to create a new JavaScript buffer. Don't create it directly, use the
...
// Buffer is a type that simplifies the generation of JavaScript code.
type Buffer struct {
	reporter  *reporter.Reporter
	pkg       string
	file      string
...
// NewBuffer creates a builder for JavaScript buffers.
...
// File sets the name of the file, without the .js extension.
...
func (b *BufferBuilder) Build() (buffer *Buffer, err error) {
	...
	buffer.file = filepath.Join(b.output, b.pkg, b.file+".js")
...
// that generates the code of a JavaScript method could be used like this:
//
...
func (b *Buffer) Write() error {
	...
	if len(b.imports) > 0 {
		for imprt, selector := range b.imports {
			fmt.Fprintf(outputFd, "import {%s} from \"%s\";\n", selector, imprt)
		}
		fmt.Fprintf(outputFd, "\n")
	}
...
// cleanPkg transforms the given text so that it is a valid JavaScript package name. This means removing the
// `-js` suffix if present, and removing any dashes. For example, the string `ocm-sdk-js` will be
// translated into `sdk`.
